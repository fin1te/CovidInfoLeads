...
public class Plasma extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingSearchView floatingSearchView = findViewById(R.id.plasmasearch);
        ...
        plasmarecview=(RecyclerView)findViewById(R.id.plasmarecview);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this);
//        linearLayoutManager.setReverseLayout(true);
//        linearLayoutManager.setStackFromEnd(true);
        plasmarecview.setLayoutManager(new LinearLayoutManager(this));

        BottomNavigationBar bottomNavigationBar = (BottomNavigationBar) findViewById(R.id.bottom_navigation_barplasma);
        ...
        bottomNavigationBar.setTabSelectedListener(new BottomNavigationBar.OnTabSelectedListener(){
            ...
            public void onTabSelected(int position) {
                if(position==0) {
                    Intent intent = new Intent(Plasma.this, Vaccine.class);
                    startActivity(intent);
                    finish();
                }
                else if(position==1)
                    plasmarecview.scrollToPosition(position-1);
                else if(position==2) {
                    Intent intent = new Intent(Plasma.this, CovidData.class);
                    startActivity(intent);
                    finish();
                }
                else if(position==3) {
                    Intent intent = new Intent(Plasma.this, Dashboard.class);
                    startActivity(intent);
                    finish();
                }
            }
        ...
        FirebaseRecyclerOptions<model> options =
                new FirebaseRecyclerOptions.Builder<model>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Plasma").orderByChild("reversetoken"), model.class)
                        .build();
        adapter = new myadapter(options);
        plasmarecview.setAdapter(adapter);
    }
    @Override
    ...
    private void searchprocess(String s) {
        ...
        FirebaseRecyclerOptions<model> options =
                new FirebaseRecyclerOptions.Builder<model>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Plasma").orderByChild("cityName").startAt(s.toString()).endAt(s.toString()+"\uf8ff"), model.class)
                        .build();
        adapter = new myadapter(options);
        plasmarecview.setAdapter(adapter);
        adapter.startListening();

...
public class Medicines extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingSearchView floatingSearchView = findViewById(R.id.medsearch);
...
//        linearLayoutManager.setStackFromEnd(true);
        ...
        bottomNavigationBar.setTabSelectedListener(new BottomNavigationBar.OnTabSelectedListener(){
            ...
            public void onTabSelected(int position) {
                if(position==0) {
                    ...
                    Intent intent = new Intent(Medicines.this, CovidData.class);
                    startActivity(intent);
                    finish();
                }
                else if(position==3) {
    ...
    private void searchprocess(String s) {
        ...
        FirebaseRecyclerOptions<model> options =
                new FirebaseRecyclerOptions.Builder<model>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Medicines").orderByChild("cityName").startAt(s.toString()).endAt(s.toString()+"\uf8ff"), model.class)
                        .build();
        adapter = new myadapter(options);
        medsrecview.setAdapter(adapter);
        adapter.startListening();

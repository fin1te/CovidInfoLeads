...
public class Other extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingSearchView floatingSearchView = findViewById(R.id.othersearch);
...
//        linearLayoutManager.setStackFromEnd(true);
    ...
    private void searchprocess(String s) {
        ...
        FirebaseRecyclerOptions<model> options =
                new FirebaseRecyclerOptions.Builder<model>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Other").orderByChild("cityName").startAt(s.toString()).endAt(s.toString()+"\uf8ff"), model.class)
                        .build();
        adapter = new myadapter(options);
        othrecview.setAdapter(adapter);
        adapter.startListening();
